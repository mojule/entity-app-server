...
const noop = () => { };
const FileCreateStorageFactory = (db, diskFileDeps, imageFileDeps, zipFileDeps, options = {}) => {
    const { isExtractOnly = false, overwriteExisting = false } = options;
    const diskFile = (fileData) => createDiskFile(db.collections.file, fileData, diskFileDeps.getStaticPath, diskFileDeps.getRootPath, diskFileDeps.validator || noop, overwriteExisting);
    const imageFile = (fileData) => createImageFile(db.collections.imageFile, fileData, imageFileDeps.getStaticPath, imageFileDeps.getRootPath, imageFileDeps.validator || noop, overwriteExisting);
    const zipFile = async (fileData) => {
        const { mimetype, size, tags } = fileData;
        const { destFilePath, uriPath } = getPaths(fileData, zipFileDeps.getStaticPath, zipFileDeps.getRootPath, overwriteExisting);
        let bufferMap;
        let buffer;
        ...
        const paths = await Promise.all(originalPaths.map(async (originalPath) => {
            ...
            if (is_image_mime_1.isImageMime(mimetype)) {
                const imageId = await createImageFile(db.collections.imageFile, fileData, imageFileDeps.getStaticPath, getRootPath, imageFileDeps.validator || noop, overwriteExisting);
                const ref = {
            ...
            const fileId = await createDiskFile(db.collections.file, fileData, diskFileDeps.getStaticPath, getRootPath, diskFileDeps.validator || noop, overwriteExisting);
            const ref = {
                _collection: 'file',
                _id: fileId
            };
...
const createFileEntity = (fileData, staticPath, rootPath, overwriteExisting) => {
    const { mimetype, size, tags } = fileData;
...
const createDiskFile = async (collection, fileData, staticPath, rootPath, validator, overwriteExisting) => {
    const { buffer } = fileData;
    if (buffer === undefined) {
        throw Error(`Expected buffer`);
    }
    const { destFilePath } = getPaths(fileData, staticPath, rootPath, overwriteExisting);
    const diskFile = createFileEntity(fileData, staticPath, rootPath, overwriteExisting);
...
const createImageFile = async (collection, fileData, staticPath, rootPath, validator, overwriteExisting) => {
    const { buffer } = fileData;
    if (buffer === undefined)
        throw Error(`Expected buffer`);
    const { imageFile, image } = await process_image_1.processImageFileData(fileData);
    const { mimetype } = imageFile.meta;
    const { destFilePath, uriPath } = getPaths(fileData, staticPath, rootPath, overwriteExisting);
...
const getPaths = (fileData, staticPath, rootPath, overwriteExisting) => {
    const { originalname } = fileData;
    const destPath = join(staticPath(fileData), rootPath(fileData));
    const destName = (overwriteExisting ?
        join(destPath, originalname) :
        next_filename_1.nextFilename(destPath, originalname));
